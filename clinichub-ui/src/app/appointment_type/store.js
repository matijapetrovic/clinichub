...
export default {
  ...
  actions: {
    ...
    changeAppointmentType({ commit, dispatch }, { id, appointmentType }) {
      return api.changeAppointmentType(id, appointmentType)
        .then((response) => {
          commit('CHANGE_APPOINTMENT_TYPE', response.data);
          const message = 'Appointment type updated successfully!';
          dispatch('notifications/add', utils.successNotification(message), { root: true });
        })
        .catch((err) => {
          dispatch('notifications/add', utils.errorNotification(err), { root: true });
