...
export default {
  ...
  mutations: {
    ...
    UPDATE_DOCTOR(state, updatedDoctor) {
      state.doctors = state.doctors.map((doctor) => (doctor.id === updatedDoctor.id
        ? updatedDoctor
        : doctor));
    },
  },
  actions: {
    ...
    getAllDoctorsForClinic({ commit }, clinicId) {
      return api.getAllDoctorsForClinic(clinicId)
        .then((data) => {
          commit('SET_DOCTORS', data.data);
        });
    },
    ...
    updateDoctor({ commit, dispatch }, { doctorId, updatedDoctor }) {
      return api.updateDoctor(doctorId, updatedDoctor)
        .then((response) => {
          const message = 'Doctor updated successfully';
          dispatch('notifications/add', utils.successNotification(message), { root: true });
